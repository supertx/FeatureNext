...
class GaborAugDataset(Dataset):
    ...
    @staticmethod
    def gabor_filter_bank(img: torch.Tensor, scales=[4, 8], kernel_size=[(15, 15), (21, 21)], orientations=8) ->torch.Tensor:
        responses = [img]
        gray_img = torch.mean(img, dim=0, keepdim=True).unsqueeze(0)  # [1, H, W]
        for lam, ks in zip(scales, kernel_size):
            for theta in np.linspace(0, np.pi, orientations, endpoint=False):
                kernel = cv.getGaborKernel(ks, sigma=lam/2, theta=theta, lambd=lam, gamma=0.5)
                kernel = torch.from_numpy(kernel).float().unsqueeze(0).unsqueeze(0)  # [1, 1, kH, kW]
                filtered = torch.nn.functional.conv2d(gray_img, kernel, padding='same')  # [1, 1, H, W]
                filtered = filtered.squeeze(0)  # [1, H, W]
                responses.append(filtered)
        return torch.cat(responses, dim=0)  # [3 + scales * orientations, H, W]
    ...
    def __gen_roi(
        ...
        if scale_disturb:
            scale = 1.0 + np.random.uniform(0, 0.5)
        else:
            scale = 1.0
    ...
    def __getitem__(self, index):
        ...
        if len(dis_roi) > 0:
            ...
            rois = [GaborAugDataset.gabor_filter_bank(roi) for roi in rois]
            rois = torch.stack(rois, dim=0)
        else:
            rois = self.transform(raw_roi)
            rois = GaborAugDataset.gabor_filter_bank(rois)
        return rois, label
...
if __name__ == "__main__":
    dataset = GaborAugDataset(
        img_root="/data/tx/palm_data/IITD/resized",
        anno_file="/data/tx/palm_data/IITD/annotations/fourk_val.json",
        roi_size=(128, 128),
        scale_disturb=True,
        transform=T.Compose([
            T.ToTensor(),
