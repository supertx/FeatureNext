...
class PalmprintFCMAE(nn.Module):
    def __init__(self, mask_ratio=0.6):
        ...
        # Encoder
        print("正在加载 ConvNeXt Tiny 预训练权重...")
...
def train_one_epoch(model, dataloader, optimizer, device, epoch_idx):
    ...
    for batch_idx, (images, _) in enumerate(dataloader):
        ...
        if batch_idx % 10 == 0:
            print(f"Epoch [{epoch_idx}] Step [{batch_idx}/{len(dataloader)}] "
                  f"Loss: {loss.item():.4f} (MSE: {loss_mse.item():.4f}, Gabor: {loss_gabor.item():.4f})")
            
    return total_loss_avg / len(dataloader)

import matplotlib.pyplot as plt

def save_reconstruction_images(model, dataloader, device, epoch, save_dir, n_imgs=4):
    """
    ...
    # 创建保存目录
    vis_dir = os.path.join(save_dir, "vis_epoch")
...
def palm_pretrain():
    ...
    # 保存最终模型
    torch.save(model.state_dict(), os.path.join(save_dir, "palm_fcmae_final.pth"))
    print("训练完成！")